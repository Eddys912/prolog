...
# Usage: make run file=exercises/<file.pl>
...
run:
	@if [ -z "$(file)" ]; then \
		...
		printf "$(INFO) $(CYAN) Usage: make run file=exercises/<file.pl> $(NC)\n"; \
		exit 1; \
	fi
...
help:
	...
	@printf "$(INFO) $(CYAN) Example: make run exercises/01_animal_hierarchy.pl $(NC)\n"
